...
static void* os_timer_kqueue_handle(void *arg) {
	...
	excimer_os_timer_notify_function_t* notify_function = timer->notify_function;
	int kq = timer->kq;
...
int excimer_os_timer_start(excimer_os_timer_t* os_timer, struct timespec *period, struct timespec *initial) {
	...
	EV_SET(&os_timer->kev, os_timer->id, EVFILT_TIMER, EV_ADD | EV_ENABLE, NOTE_NSECONDS, timer_period_nanos, (void*)os_timer->id);
